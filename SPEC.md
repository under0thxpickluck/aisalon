...
`?id={predictionId}` — Replicate の予測状況をポーリング。

**完了時の処理:**
1. Replicate の出力URLから音声バイナリをダウンロード
2. Cloudflare R2 に `bgm/{predictionId}.wav` としてアップロード（永続保存）
3. R2 の公開URLを返す。R2アップロード失敗時は Replicate URL にフォールバック
