...
// plan列 → BP上限（GAS planToGrant_ 実値 + /5000プラン）
...
export default function MembershipPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] text-slate-900 dark:text-white px-4 py-8 max-w-lg mx-auto">

      {/* ヘッダー */}
      <div className="flex items-center justify-between mb-6">
        <Link href="/top" className="text-slate-500 dark:text-white/40 text-sm">← Back</Link>
        <div className="text-center">
          <h1 className="font-bold text-lg">メンバーシップ & BP</h1>
          <p className="text-xs text-slate-500 dark:text-white/40">クレジットの確認・回復・追加購入</p>
        </div>
        <div className="w-16" />
      </div>
        ...
        {loading ? (
          ...
          <>
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-xs text-slate-500 dark:text-white/40 mb-1">現在のランク</p>
                <p className="font-black text-xl text-purple-400">{status?.rank ?? "Starter"}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-slate-500 dark:text-white/40 mb-1">次回更新日</p>
                <p className="text-sm text-slate-600 dark:text-white/70">{status?.next_renewal ?? "—"}</p>
              </div>
            </div>

            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="bg-slate-200 dark:bg-white/5 rounded-xl p-3 text-center">
                <p className="text-xs text-slate-500 dark:text-white/40 mb-1">通常BP</p>
                <p className="font-black text-lg text-slate-900 dark:text-white">{(status?.base_bp ?? 0).toLocaleString()}</p>
              </div>
              <div className="bg-slate-200 dark:bg-white/5 rounded-xl p-3 text-center">
                <p className="text-xs text-slate-500 dark:text-white/40 mb-1">追加BP</p>
                <p className="font-black text-lg text-blue-400">{(status?.extra_bp ?? 0).toLocaleString()}</p>
              </div>
              <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3 text-center">
                <p className="text-xs text-slate-500 dark:text-white/40 mb-1">合計BP</p>
      ...
      {/* BPの仕組み */}
      <div className="bg-slate-100 dark:bg-white/5 rounded-xl p-5 mb-6">
        ...
        <div className="grid grid-cols-2 gap-2 mt-3">
          <div className="bg-slate-200 dark:bg-white/5 rounded-lg p-3 text-center">
            <p className="text-xs text-slate-500 dark:text-white/40">通常クレジット</p>
            <p className="text-sm font-bold text-green-400 mt-1">毎月回復</p>
          </div>
          <div className="bg-slate-200 dark:bg-white/5 rounded-lg p-3 text-center">
            <p className="text-xs text-slate-500 dark:text-white/40">追加クレジット</p>
      ...
      <div className="bg-slate-100 dark:bg-white/5 rounded-xl p-5 mb-6">
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="flex items-center justify-between text-sm bg-slate-200/60 dark:bg-white/3 rounded-lg p-3">
              <span className="text-slate-500 dark:text-white/40">現在 {c.current}</span>
              <span className="text-green-400">+{c.recovery}</span>
              <span className="text-slate-900 dark:text-white font-bold">→ {c.result}</span>
              <span className="text-slate-400 dark:text-white/30 text-xs">{c.note}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* ランクについて */}
      <div className="bg-slate-100 dark:bg-white/5 rounded-xl p-4 mb-6">
      ...
      <div id="purchase" className="mb-6">
        ...
        <p className="text-xs text-slate-500 dark:text-white/40 mb-1">高品質なAI処理・インフラ維持のため、BP価格を調整しています</p>
        <p className="text-sm text-slate-600 dark:text-white/60 mb-4">追加購入したBPは失効せず、回復の影響も受けません。</p>

        <div className="space-y-3">
          {BP_PACKS.map(pack => (
            <div key={pack.id}
              className={`border ${pack.color} rounded-xl p-4 bg-slate-100 dark:bg-white/5 relative`}>
              {pack.tag && (
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-black text-2xl text-slate-900 dark:text-white">{pack.bp.toLocaleString()} <span className="text-sm text-slate-500 dark:text-white/40">BP</span></p>
                  <p className="text-sm text-slate-600 dark:text-white/60 mt-0.5">{pack.label}パック</p>
                </div>
                <div className="text-right">
      ...
      {msg && (
        <div className="bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-3 text-sm text-slate-600 dark:text-white/70 mb-4 text-center">
          {msg}
        </div>
      )}
      ...
      <div className="bg-slate-100 dark:bg-white/5 rounded-xl p-4 mb-6">
        ...
        <ul className="text-xs text-slate-500 dark:text-white/40 space-y-1">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mt-4">
        <a href="#purchase"
          className="flex-1 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-blue-600 text-center font-bold text-white">
          BPを購入
        </a>
        <Link href="/top"
          className="flex-1 py-3 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-center font-bold text-slate-600 dark:text-white/60">
          利用を開始
        </Link>
      </div>
    </div>
