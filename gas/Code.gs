...
function handle_(key, body) {
  ...
  if (action === "affiliate_monthly_summary") {
    ...
    // --- CC affiliate rewards from wallet_ledger (kind="cc_affiliate_reward") ---
  ...
  if (action === "my_referral_dashboard") {
    ...
    // - 旧制度（USD建て）: referral_bonus / referral_entry → total_bonus に合算
    // - EP建て（クレカ紹介・2026-07〜）: cc_affiliate_reward
    ...
    if (ledger_rd) {
      ...
      if (ledValues.length >= 2) {
        ...
        const BONUS_KINDS = ["referral_bonus", "referral_entry", "cc_affiliate_reward"];
        const EP_KINDS    = ["cc_affiliate_reward"];
        for (let i = 0; i < lRows.length; i++) {
          const r = lRows[i];
...
// =====================================================
//
// 呼び元: /api/sticker/* （app/lib/sticker/gas.ts 経由）
//
// 設計上の約束:
//   credits 列は stickerCredits_ だけが書き換える。
//   stickerSave_ は credits を一切触らない。
//   これによりクライアントがクレジットを増やすことができない。
